...
## 최우선 규칙: 출제 유형 제한 (다른 모든 규칙보다 우선)
options의 question_types 배열을 확인하세요.

[question_types가 비어있는 경우]
→ 학교 출제 패턴대로 자유롭게 출제

[question_types에 유형이 있는 경우] ← 이 경우가 가장 중요
→ 해당 유형 외에는 단 한 문제도 출제하지 마세요
→ 예시: question_types = ["흐름에 맞지 않는 문장", "빈칸추론"]
   - 올바른 예: 모든 객관식이 "흐름에 맞지 않는 문장"과 "빈칸추론"으로만 구성
   - 잘못된 예: 어휘, 어법, 주제, 요지, 제목, 순서배열 등 다른 유형 출제 → 절대 금지
→ question_types에 없는 유형은 아무리 지문이 적합해 보여도 출제하지 마세요

## 출제 규칙
1. 반드시 제공된 지문의 본문만 사용하세요
2. 원본 지문의 유형을 그대로 재사용하지 마세요
   - 빈칸추론이었던 지문 → 위 최우선 규칙에서 허용된 유형으로만 변환
3. 학교 출제 패턴의 변형 방식을 따르되, question_types 제한을 우선시하세요
...
7. 배점 계산 규칙
   ...
   - 객관식은 동일 배점으로 설정
8. 정답 번호 분포 규칙
   - 전체 문제의 정답이 ①~⑤에 고르게 분포되도록 하세요
   - 같은 번호가 3회 이상 연속으로 정답이 되지 않도록 하세요
   - 예: 30문제면 각 번호당 약 6문제씩 정답이 되도록 설계
8-1. 유형별 정답 번호 제한 규칙
   - 흐름에 맞지 않는 문장: ①, ⑤ 정답 불가
   - 문장 삽입: ① 정답 불가
   - 글의 순서: ① 정답 불가
...
## 서술형 출제 규칙
1. question_types에 서술형 유형이 지정된 경우 해당 유형만 출제
2. 학교 출제 패턴의 서술형 조건 형식을 따르세요 (이니셜, 보기 단어, 조건 수 등)
3. 서술형은 choices 없이 condition만 포함
