...
## ⚠️ 최우선 규칙: 출제 유형 제한 (다른 모든 규칙보다 우선)
options의 question_types 배열을 확인하세요.

[question_types가 비어있는 경우]
→ 학교 출제 패턴대로 자유롭게 출제

[question_types에 유형이 있는 경우]
...
### 빈칸추론
- 지문의 핵심 논리/주제를 담은 문장에 빈칸 설정
- 빈칸 위치는 지문의 첫 문장, 중간 문장, 마지막 문장 어디든 가능
- 단, 빈칸이 있는 문장에서 빈칸이 문장의 맨 첫 단어로 시작하면 안 됨
- 빈칸은 핵심 어구 또는 절 단위로 설정 (단어 1개는 지양)
- 선택지는 지문 표현을 그대로 쓰지 말고 반드시 paraphrase하거나 완전히 새로운 표현으로
- 정답은 지문 전체 맥락을 이해해야 고를 수 있도록 설계
- 오답은 지문 일부 내용과 관련있지만 핵심 논리를 벗어난 것으로
- 발문: "다음 빈칸에 들어갈 말로 가장 적절한 것은?"
- 지문에서 핵심 문장 한 곳에만 ______ 표시
- 선택지 5개는 모두 paraphrase된 완성된 어구/절

### 흐름에 맞지 않는 문장
- 지문을 5~7개 문장으로 구성하고 각 문장 앞에 ①~⑤ (또는 ①~⑦) 번호 부여
- 정답 문장은 지문 주제와 관련있어 보이지만 논리적으로 맞지 않는 내용으로 삽입
- ①, ⑤ (마지막 번호) 는 절대 정답 불가
- 발문: "다음 글에서 전체 흐름과 관계 없는 문장은?"

### 문장 삽입
- 삽입할 문장 제시 후 지문에 ( ① )~( ⑤ ) 위치 표시
- 첫 번째 위치 ( ① ) 는 절대 정답 불가
- 발문: "글의 흐름으로 보아, 주어진 문장이 들어가기에 가장 적절한 곳을 고르시오."

### 글의 순서
- 첫 문단 제시 후 (A)(B)(C) 세 단락을 섞어서 제시
- 선택지:
  ① (A)-(B)-(C)
  ② (A)-(C)-(B)
  ③ (B)-(A)-(C)
  ...
  ⑤ (C)-(B)-(A)
- ① (A)-(B)-(C) 는 정답 불가
- 발문: "주어진 글 다음에 이어질 글의 순서로 가장 적절한 것을 고르시오."

### 어법
- 지문 내 5개 어구/표현에 밑줄을 치고 그 중 어법상 틀린 것 1개 고르기
- 밑줄은 단어 또는 짧은 어구 단위로
- 오답 4개는 어법상 올바른 표현
- 정답 1개는 어법상 틀린 표현 (준동사/관계사/수동태/시제/수일치 등)
- 발문: "다음 글의 밑줄 친 부분 중, 어법상 틀린 것은?"

### 어휘 (문맥에 맞지 않는 단어)
- 지문 내 5개 단어에 밑줄을 치고 그 중 문맥상 적절하지 않은 것 1개 고르기
- 정답 단어는 반의어 또는 전혀 다른 의미의 단어로 교체
- 오답 4개는 문맥상 올바른 단어
...
3. 선택지는 반드시 5개, 정답은 1개
4. 난이도가 null이면 학교 기본 난이도 유지
5. 듣기평가 문제는 절대 출제하지 마세요
6. 배점 계산 규칙
   ...
   - 객관식은 동일 배점으로 설정
7. 정답 번호 분포 규칙 (절대 준수)
   - 문제를 출제하기 전에 먼저 각 문제의 정답 번호를 균등하게 배분하세요 (매우 중요)
   - 전체 문제에서 ①~⑤ 정답이 균등하게 분포되도록 설계
   - 예: 40문제면 각 번호당 약 8문제씩 정답
   - 같은 번호가 2회 이상 연속으로 정답이 되지 않도록 하세요
   - 유형별 제한이 있는 경우 해당 제한 안에서 균등 분포
     예: 흐름 문제만 있으면 ②③④ 중에서 균등 분포
